...
RUNNER_NAME="${RUNNER_NAME:-dockerNode}-${RUNNER_SUFFIX}"
...
###
# validate runner enpoint
###
if [ -n "${RUNNER_ORG}" ] && [ -n "${RUNNER_REPO}" ] ; then
  ATTACH="${RUNNER_ORG}/${RUNNER_REPO}"
  TOKEN_API_URL="https://api.github.com/repos/${ATTACH}/actions/runners/registration-token"
elif [ -n "${RUNNER_ORG}" ]; then
  ATTACH="${RUNNER_ORG}"
  TOKEN_API_URL="https://api.github.com/orgs/${ATTACH}/actions/runners/registration-token"
elif [ -n "${RUNNER_REPO}" ]; then
  ATTACH="${RUNNER_REPO}"
else
  log.error 'At least one of RUNNER_ORG, RUNNER_REPO must be set'
  exit 1
fi

###
# check authentication mechanism and test authentication
###
if [ -n "${RUNNER_APP_SECRET}" ] && [ -n "${RUNNER_APP_ID}" ] && [ -n "${RUNNER_PAT}" ] ; then
  log.error 'either app credentials (RUNNER_APP_SECRET and RUNNER_APP_ID) or user credentials (RUNNER_PAT) need to be set. not both.'
  exit 1
elif [ -n "${RUNNER_PAT}" ] ; then
  #
  # user based authentication
  #
  log.debug 'authenticating as user'
  ...
  log.error 'either app credentials (RUNNER_APP_SECRET and RUNNER_APP_ID) or user credentials (RUNNER_PAT) need to be set.'
  exit 1
fi

RUNNER_TOKEN=$(helper.org.runner.registrationtoken "${RUNNER_ORG}" "$RUNNER_PAT")
